...
export default function InstallmentsScreen() {
  ...
  const cardInstallmentSummaries = useMemo(() => cards
    .filter((card) => card.type === "credit")
    .map((card) => {
      const cardInstallments = active.filter((item) => item.cardId === card.id);
      const totalRemaining = cardInstallments.reduce((sum, item) => {
        const remainingInstallments = item.totalInstallments - item.paidInstallments;
        return sum + (item.installmentAmount / item.totalInstallments) * remainingInstallments;
      }, 0);
      const monthlyPayment = cardInstallments.reduce(
        (sum, item) => sum + item.installmentAmount / item.totalInstallments,
        0
      );

      return { card, count: cardInstallments.length, totalRemaining, monthlyPayment };
    })
    .filter((summary) => summary.count > 0), [active, cards]);
  ...
  async function save() {
    ...
    try {
      setSaving(true);
      const input = { name: name.trim(), cardId, totalInstallments, paidInstallments, installmentAmount: amount, note: note.trim() || undefined };
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
    ...
    {cardInstallmentSummaries.length > 0 && <View style={styles.summarySection}><Text style={styles.sectionTitle}>Kart Bazında Taksit Özeti</Text><View style={styles.summaryList}>{cardInstallmentSummaries.map(({ card, count, totalRemaining, monthlyPayment }) => <View key={card.id} style={styles.cardSummary}><Text style={styles.cardSummaryTitle}>{card.bankName} · {card.name}</Text><Text style={styles.cardSummaryCount}>{count} devam eden taksit</Text><View style={styles.cardSummaryAmounts}><View><Text style={styles.cardSummaryLabel}>KALAN TOPLAM TAKSİT</Text><Text style={styles.cardSummaryValue}>{formatMoney(totalRemaining)}</Text></View><View style={styles.cardSummaryAmountRight}><Text style={styles.cardSummaryLabel}>AYLIK TOPLAM ÖDEME</Text><Text style={[styles.cardSummaryValue, styles.monthlyValue]}>{formatMoney(monthlyPayment)}</Text></View></View></View>)}</View></View>}
