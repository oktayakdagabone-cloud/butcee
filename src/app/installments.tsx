...
export default function InstallmentsScreen() {
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
    ...
    <View style={styles.form}><Text style={styles.formTitle}>{editing ? "Taksiti Düzenle" : "Yeni Taksit Ekle"}</Text>
      ...
      <Text style={styles.label}>Aylık taksit tutarı (₺)</Text><TextInput value={installmentAmount} onChangeText={setInstallmentAmount} keyboardType="decimal-pad" placeholder="Örn. 1.250" placeholderTextColor="#94A3B8" style={styles.input} />
      {!!Number(installmentAmount.replace(",", ".")) && <Text style={styles.helper}>Kart harcaması: {(Number(installmentAmount.replace(",", ".")) * Number(total || 0)).toLocaleString("tr-TR")} ₺ · kart limitinden düşer.</Text>}
